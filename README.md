...
SD카드에 저장되는 데이터를 최대 파일 개수를 설정하여 최대 파일 개수에 도달하면 오래된 순서대로 삭제되는 코드이지만, 현재 파일 열기 실패 오류가 발생함.
SD카드 용량 때문에 생기는 문제일 수 있어, 다른 SD카드를 구매, 추후 개선 예정.(prevent_SUA.ino의 'amend error' commit 참조) - 김영우
